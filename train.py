...
structure_context_size = 8
structure_negative_context_size = 1

same_semantic_context_size = 6
other_semantic_context_size = 2

same_semantic_negative_context_size = 1
other_semantic_negative_context_size = 1
...
global_semantic_loss = (tf.reduce_sum((imgs_decoded-imgs_labels_placeholder)*(imgs_decoded-imgs_labels_placeholder))
                        + tf.reduce_sum((text_decoded-text_labels_placeholder)*(text_decoded-text_labels_placeholder)))
...
losses_skip = [
    [
       -context_alphas[j]*context_signs[i//2]*tf.sigmoid(context_signs[i//2]*tf.reduce_sum(tf.multiply(utils.tf_repeat(input_encodings[i%2],context_sizes[i//2][j]),context_encodings[i][j]))) for j in range(3)
    ] for i in range(4)
]
...
for losses in losses_skip:
    for loss in losses:
        loss_positive_skip = tf.add(loss_skip,loss)
...
for iteration in range(nb_iterations):
    ...
    for batch in range(len(data_imgs)//batch_size):
        ...
        batch_loss, _ = sess.run([loss,update_loss],feed_dict = input_dict)
